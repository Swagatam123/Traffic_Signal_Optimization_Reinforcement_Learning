...
def updateA(state,currentTrafficState,prevTrafficState):
    ...
    index = int((sheet3.cell_value(sheet3.nrows-2, 0) + 1))
    index2 = int((sheet3.cell_value(sheet3.nrows - 1, 0) + 1))
    #print((int(sheet3.cell_value(sheet3.nrows-1, 5)+65)))
    c = chr(int(sheet3.cell_value(sheet3.nrows-2, 5)+64)) + str(index)
    c1 = chr(int(sheet3.cell_value(sheet3.nrows - 1, 5) + 64)) + str(index2)
    #print("q",c)
    #reward=calculateReward(currentTrafficState,prevTrafficState)
    reward = sheet3.cell_value(sheet3.nrows - 2, 7)
    #print(reward)
    # m = []
    # for i in range(12):
   ...
   # print(alpha*(reward+gamma*max(m))+(1-alpha)*int(sheet[c].value))
    #print(sheet[c].value)
    sheet[c]=alpha*(reward+gamma*int(sheet[c1].value))+(1-alpha)*int(sheet[c].value)
